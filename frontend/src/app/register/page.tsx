    ...
    try {
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        ...
        if (axiosErr.response?.data?.errors) {
          setError(axiosErr.response.data.errors[0].message);
        } else {
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-xl shadow-elevated">
            <div className="mb-xl text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-primary-fixed mb-md">
                <span className="material-symbols-outlined text-primary text-2xl" data-icon="person_add">person_add</span>
              </div>
              <h1 className="font-h1 text-primary">Create Account</h1>
              <p className="text-secondary font-body-sm mt-xs">Start managing your expenses today</p>
            </div>
            ...
            <p className="text-center text-body-sm text-secondary">
              ...
              <Link href="/login" className="font-label-md text-primary hover:underline decoration-primary/30 underline-offset-4">
                Log In
              </Link>
            </p>
          </div>
